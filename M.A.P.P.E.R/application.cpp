...
void application::register_default_buttons_callbacks(ezgl::application *application)
{
  ...
  // Connect press_findDirections function to the Directions button
  GObject *FindDirections_button = application->get_object("FindDirections");
  g_signal_connect(FindDirections_button, "clicked", G_CALLBACK(press_find_directions), application);

  // Connect press_help function to the Help button
  GObject *help_button = application->get_object("HELP");
  g_signal_connect(help_button, "clicked", G_CALLBACK(press_help), application);
  
  // Connect filter for intersections
  GObject *AutoIntersections_filter = application->get_object("AutoIntersections");
  g_signal_connect(AutoIntersections_filter, "clicked", G_CALLBACK(press_auto_intersections), application);
  
  // Connect filter for streets
  GObject *AutoStreets_filter = application->get_object("AutoStreets");
  g_signal_connect(AutoStreets_filter, "clicked", G_CALLBACK(press_auto_streets), application);
